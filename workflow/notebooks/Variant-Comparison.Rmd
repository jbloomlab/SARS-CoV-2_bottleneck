...
for (i in 1:length(accessions.to.include)){
  for (j in 1:length(accessions.to.include)){
    ...
    variants.i = filter(combined.include.df.indels, Accession == accession.i)$SNP.one
    variants.j = filter(combined.include.df.indels, Accession == accession.j)$SNP.one
...
results %>% 
  ggplot(aes(x = accession.i, y = accession.j, fill = n_intersect)) + 
    geom_tile() +
