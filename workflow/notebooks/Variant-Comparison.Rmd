...
pileup.filepath = "../../results/pileup/raw-variants.csv" # Variants called from pileup files
...
# Reshape the variant data to combine replicates
combined.pileup.df = inner_join(filter(pileup.df, Replicate == 1),
...
inner_join(combined.pileup.df,
                  combined.df,
                  by = c( "Accession", "POS", "REF", "ALT"),
                  suffix = c(".pileup", ".varscan")) %>% 
  filter(DP.one.pileup > 200) %>% 
  ggplot(aes(x = log(AF.one.pileup), y = log(AF.one.varscan))) +
    geom_point() +
    geom_smooth(method='lm', col = "red", se = F)
```
...
### Testing the results with mpileup parising

```{r}
output = read_csv( "/Volumes/whannon/2020/SARS-CoV-2_bottleneck/results/pileup/raw-variants.csv")
...
SNPs.df = output %>% 
  ...
  mutate(Replicate = ifelse(is.na(Replicate), 1, Replicate)) %>% 
  rename(REF = "Reference Base", CONS = "Consensus Base", POS = "Position") %>% 
  pivot_longer(cols = c("A For", "A Rev", "C For", "C Rev", "G For", "G Rev", "T For", "T Rev"), names_to = "ALT", values_to = "Count") %>% 
  separate(col = ALT, into = c("ALT", "Strand")) %>% 
  mutate(Strand = ifelse(Strand == "For", "+", "-"), SNP = paste0(REF, POS, ALT)) %>% 
  mutate(AF = Count/Depth, Type = case_when(ALT != REF & ALT != CONS ~ "SNP",
                                           ALT == REF & ALT != CONS ~ "SNP",
                                           ALT == REF & ALT == CONS ~ "REF",
                                           ALT != REF & ALT == CONS ~ "CONS")) %>% 
  filter(AF > 0.01, Depth >= 200, Type == "SNP") %>% 
  arrange(-AF)

## ---------- ##

# Reshape the variant data to combine replicates
combined.pileup.df = inner_join(filter(SNPs.df, Replicate == 1),
                        filter(SNPs.df, Replicate == 2),
                        by = c( "Accession", "POS", "REF", "ALT", "CONS", "Strand"),
                        suffix = c(".one", ".two")) %>% 
              select(!c("Replicate.one", "Replicate.two"))
...
combined.pileup.df  %>% 
  ggplot(aes(x = log10(AF.one), y = log10(AF.two), col = Strand)) + 
    facet_wrap(~Accession) +
