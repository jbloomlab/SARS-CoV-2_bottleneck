...
# TODO - it's not properly labeling the boat clades. 
...
as.treedata(tree_tibble) %>% 
  ggtree(size = 1.1, col = "darkgrey") + 
    ...
    geom_cladelabel(node = break.monophyletic.node[1], label="*",fontsize = 5, col='#55ACA6',  offset = .000001) +
    geom_cladelabel(node = break.monophyletic.node[2], label="*",fontsize = 5, col='#55ACA6',  offset = .000001) +
...
plt = collapsed.tree.plotting %>% 
  ggtree(size = 1.5, col = "black", ladderize=T, right=T) %>%
  rotate(rootnode(collapsed.tree.plotting)) +
    ...
    geom_point2(aes(subset = label == "10136"), col='darkred', size = 3) + 
    scale_size_continuous(name ="Genome Count", breaks = c(1, 3, 10), range = c(3,10)) +
    labs(col="") 
...
ggsave("../../results/figures/representative-global-phylogeny.png", width = 22, height = 12, dpi = 300)
ggsave("../../results/figures/representative-global-phylogeny.svg", width = 22, height = 12)
