...
Donut plot of samples

```{r Donut, echo = F, message = F, warning = F, fig.width=6, fig.height=6, fig.align='center'}

donut.df = data.frame(Category = c("Not Tested", "Negative", "Postive + Low Quality", "Positve + Deep-Sequenced"),
                      Count = c(4, 20, 59, 39)) %>% 
...
qc.before.merge %>% 
  filter(library == "1") %>% 
  select(sample, replicate, library, percent_filtered_viral_reads) %>% 
  pivot_wider(names_from = replicate, values_from = percent_filtered_viral_reads) %>% 
  mutate(percent_diff = abs(`1` - `2`)) %>% 
  pivot_longer(!c("sample", "library", "percent_diff"), names_to = "replicate", values_to = "percent_filtered_viral_reads") %>%
  ggplot(aes(x = reorder(as.factor(sample), -percent_diff), y = percent_filtered_viral_reads, fill = as.factor(replicate))) +
    geom_bar(stat = "identity", position = position_dodge()) + 
    scale_fill_manual(values = experiment.colors, name = "Replicate") +
...
```{r Percent Filtered library, echo = FALSE, message = FALSE, fig.width=11, fig.height=5, fig.align='center'}

qc.before.merge %>% 
  ggplot(aes(x = (as.factor(sample)), y = percent_filtered_viral_reads, fill = as.factor(library))) +
    geom_bar(stat = "identity", position = position_dodge()) + 
    scale_fill_manual(values = experiment.colors, name = "Replicate") +
    scale_y_continuous(labels = percent) + 
    xlab("Sample") +
...
```{r Average Base Quality, echo = FALSE, message = FALSE, fig.width=11, fig.height=5, fig.align='center'}
