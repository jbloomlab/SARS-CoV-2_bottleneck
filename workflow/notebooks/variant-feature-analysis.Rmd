...
variant.combined.df = full_join(
  ...
  by = c("ACCESSION", "POS", "REF", "ALT", "Caller", "EFFECT", "GENE", "AA_CHANGE"),
  suffix = c(".one", ".two")
  ) %>% 
  select(!c("REPLICATE.one", "REPLICATE.two")) %>% 
  mutate(AF.one = if_else(is.na(AF.one), 0, AF.one)) %>% 
...
selected.variant.df = pysam.combined.df %>% 
  ...
  select(ACCESSION, POS, REF, ALT, AF, AA_CHANGE, EFFECT, GENE) 
...
caller.selected.variant.df = variant.combined.df %>% 
  filter(!(ACCESSION %in% low.coverage.samples)) %>% 
  filter(!(ACCESSION %in% poor.concordance.samples)) %>% 
...
# Filp the consensus variants
caller.fipped.cons = caller.selected.variant.df %>% 
  filter(AF >= 0.5) %>% 
  rename(ALT = "REF", REF = "ALT") %>% 
  mutate(AF = (1 - AF)) %>% 
  filter(AF >= 0.02) %>% 
...
caller.final.variant.df = rbind(filter(caller.selected.variant.df, AF < 0.5), caller.fipped.cons) %>% 
  mutate(SNV = paste0(REF, POS, ALT))
...
all.final.variants.df = rbind(caller.final.variant.df, select(mutate(final.variant.df, Caller = "pysam"), colnames(caller.final.variant.df)))
...
ggsave(filename = "../../config/figures/MAF_specturm.svg", width = 6, height = 4)
...
final.variant.df %>% 
  ...
  ggplot(aes(x = POS, y = AF, col = EFFECT, fill = EFFECT))+
    ...
    geom_bar(stat="identity", position = position_identity(), width=.01) +
...
## Everything below belongs in a different notebook. 
