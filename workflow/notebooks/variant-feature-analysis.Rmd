...
First, I'll get a list of the low coverage samples to exclude from downstream analyses. These are any samples where at least on replicate has more than 20% of the genome covered by less than 200 reads:
...
Next, I'll format the variants that I called using the pileup function of the Python library `Pysam`. I'll also join the replicates.
...
print(head(pysam.combined.snps.df))
```
...
print(head(variant.combined.df))
```

Here, I've formated the annotations of gene location and position for plotting the genome as a diagram below.
...
```{r Prepare Final Variants, warning=F, message=F}
...
Lets compare the number of SNPs called in each method to the Ct value of the initial samples. iSNVs are the SNPs that are present in both replicates with more than 100 reads covering them, more than 2% freq, SNPs only, and less than 50%.

```{r Pysam iSNVs vs. Ct, warning=F, message=F}

ct_df = read_csv(sample_metadata) %>% 
  rename("ACCESSION" = SpID, "Ct" = avg_ct) %>% 
  select(ACCESSION, Ct) %>% 
...
```{r Variant Caller iSNVs vs. Ct, warning=F, message=F}

variant.selected.variant.df = variant.combined.df %>% 
  ...
  # Allele frequency is the average of the two replicates
  mutate(AF = (AF.one + AF.two)/2) %>% 
  select(ACCESSION, POS, REF, ALT, AF, AA_CHANGE, EFFECT, GENE, Caller) 

# Filp the consensus variants
variant.fipped.cons = variant.selected.variant.df %>% 
  filter(AF >= 0.5) %>% 
  rename(ALT = "REF", REF = "ALT") %>% 
  mutate(AF = (1 - AF)) %>% 
  filter(AF >= 0.02) %>% 
...
variant.final.variant.df = rbind(filter(variant.selected.variant.df, AF < 0.5), variant.fipped.cons) %>% 
  mutate(SNV = paste0(REF, POS, ALT))

iSNV_count.df = variant.final.variant.df %>% 
