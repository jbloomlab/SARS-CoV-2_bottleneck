...
GISAID.tree.data = "../../config/representative_clades_aligned.fasta.treefile"
# Tree metadata 
metadata.data = "../../config/representative_clades_metadata.csv"
...
info = as.data.frame(read.csv(metadata.data) %>% select(!X) %>% 
                       select(id = "gisaid_epi_isl",
                              clade = "Nextstrain_clade", 
                              study))

 
ggtree(GISAID.tree, layout = "equal_angle") %<+% info +
  geom_tippoint(aes(subset=(study=="internal")), shape=1, size=2, col='red') +
  scale_color_manual(values=c("red", "green", "blue", "purple", "black", "orange")) 
