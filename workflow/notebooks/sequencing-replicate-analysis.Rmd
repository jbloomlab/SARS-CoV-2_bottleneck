...
```{r Format Pysam data, include=FALSE, warning=FALSE, message=FALSE}
...
Here I compare the variant callers to one another. I only look at SNPs because I've only benchmarked the pipeline of SNPs. It's a good sign tha the largest set is the overalp of all callers. Otherwise, there are no variants called by Pysam that are absent from other callers, which is a good tidbit. 
...
Below is a plot of the concordance of allele frequency between the sequencing replicates of all libraries. TODO: add correlation coeff. 
...
ggsave("../../config/figures/figure-2.png", plot = final_plot, width = 25, height = 20, dpi = 300, units = "in")

```

Supplemental figure showing which samples won't be included in the analysis because the allele frequences between replicates are to dissimilar. In this case, I cut off the long tail. All samples that had more than 0.025 mean difference between SNPs were not included in the downstream analysis. 
...
ggsave("../../config/figures/supplemental-figure-3.png", width = 7, height = 5, dpi = 300, units = "in")
