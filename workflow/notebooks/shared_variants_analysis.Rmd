...
# FIXME: This doesn't work with the lag function, I suspect because it's catagorical and 'lag' doesn't really mean anything without some inherent ordering. 

testdf = final.variant.df %>% 
    select(ACCESSION, SNP, AF) %>% 
    pivot_wider(names_from = SNP, values_from = AF, values_fill = 0) %>% 
    pivot_longer(!ACCESSION, names_to = "SNP", values_to = "AF") %>%
    mutate(POS = as.numeric(parse_number(SNP))) %>%
...
testdf_areaStep = bind_rows(old = testdf, 
                         new = testdf %>% mutate(AF = lag(AF)),
                         .id = "source") %>%
               arrange(ACCESSION, source)
...
for (i in 1:length(SNPs)) {
  ...
  plt = testdf %>% 
    ...
    ggplot(aes(x = reorder(ACCESSION, AF), y = AF)) +
      ...
      geom_ribbon(aes(x = ACCESSION, ymin = 0, ymax = AF), data = testdf_areaStep, fill = "black") +
