...
packages = c("tidyverse", "scales", "RColorBrewer", "ggrepel", "Biostrings")
...
```{r Shared Variants in Pysam}

shared.variants.pysam.df = shared.variants %>% 
  filter(POS %in% pull(filter(shared.variants, Caller == "pysam"), POS))

print(shared.variants.pysam.df)

```

Otherwise, there are four shared minor variants that I called with `pysam`: `r (unique(shared.variants.pysam.df$POS))`. Three out four are called with all three methods. One (`14335`) is missing from `varscan`. 

#### Low Frequnecy Shared Variants

```{r What's going on with the shared variants?}

reference = readDNAStringSet("../../config/ref/SARS2.fa")
shared.positions = unique(shared.variants.pysam.df$POS)
shared.snps = shared.variants.pysam.df %>% 
  mutate(SNP = paste0(REF, POS, ALT)) %>% 
  pull(SNP) %>% 
  unique()
...
for (i in 1:length(shared.positions)){
  ...
  print(paste("Position:", snp, "Sequence Context:", substring))
}

```

C9502T is clearly a homopolymer region. G14335T might be considered a homopolymeric? Oddly, in all four cases, the base chages to the same base as its immediate neighbor. 
